    ...
    return {
        ...
        controller: function controller($scope, $element, $timeout, Flights, Airports, Translation) {
            ...
            var getFilteredFlights = function getFilteredFlights() {
                if (_showArrivals) return _.filter(Flights.getFlights(), { ARR_DEP: "A" });else return _.filter(Flights.getFlights(), { ARR_DEP: "D" });
            };
...
app.service('Airports', function ($http, Translation) {
    ...
    var getWeather = function getWeather(index) {
        if (weatherUrl) $http.get(weatherUrl + '&lat=' + airports[index].coords.lat + '&lon=' + airports[index].coords.lon).then(function (response) {
